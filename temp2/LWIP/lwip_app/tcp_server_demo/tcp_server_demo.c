...
//			tcp_server_flag|=1<<4;//±ê¼ÇÒª·¢ËÍÊı¾İ
...
err_t tcp_server_poll(void *arg, struct tcp_pcb *tpcb)
{
	...
	if(es!=NULL)
	{
		...
		{
//			es->p=pbuf_alloc(PBUF_TRANSPORT,strlen((char*)tcp_server_sendbuf),PBUF_POOL);//ÉêÇëÄÚ´æ
...
//		pbuf_take(es->p,(char*)sendBuffer,strlen((char*)sendBuffer));
		...
		tcp_server_senddata(tcp_echoserver_pcb,es); 		//ÂÖÑ¯µÄÊ±ºò·¢ËÍÒª·¢ËÍµÄÊı¾İ
