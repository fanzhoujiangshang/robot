...
//自定义
int Sensors_I2C_Read_Register(unsigned char slave_addr,
           unsigned char reg_addr,unsigned short lenght,unsigned char *ptr)
{
  char retries=0;
  int ret = 0;
  unsigned short retry_in_mlsec = Get_I2C_Retry();
...
tryWriteAgain:  
  ret = 0;
  ret = ST_Sensors_I2C_ReadRegister( slave_addr, reg_addr, lenght ,ptr); 
